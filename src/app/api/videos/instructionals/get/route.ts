...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get all active instructional videos with creator information
    const videosResult = await query(
      `SELECT v.id, v.title, v.description, v.image_url, v.playlist_url, 
       ...
       WHERE v.is_active = true AND v.type = 'instructional'
    ...
    console.error('Error getting instructional videos:', error);
    return NextResponse.json(
