...
import { TypeSelector } from '../../create-video/components/type-selector';
...
export default function UpdateVideoPage() {
      ...
      try {
        ...
        if (data.success && data.video) {
          ...
          setFormData({
            ...
            type: video.type || '',
            author: video.author || '',
            numberOfVideos: video.number_of_videos ? video.number_of_videos.toString() : '',
            labels: video.labels || [],
            newLabel: ''
          });
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/videos/update?videoId=${videoId}`, {
        method: 'PUT',
        headers: {
    ...
    return (
      <div className="bg-[#0a0a0a] flex items-center justify-center mt-12 mb-12">
        <div className="w-full max-w-4xl px-4">
          <Card className="bg-[#1a1a1a] border border-[#2a2a2a] shadow-xl">
            <div className="p-8">
              <h1 className="text-2xl font-semibold mb-6 text-white">Error</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="pt-4 flex gap-4">
                <button
                  type="submit"
                  disabled={loading}
                  className="flex-1 flex justify-center py-2 px-4 rounded-md shadow-sm text-sm font-medium text-white bg-gray-800 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-700 disabled:bg-gray-600 disabled:cursor-not-allowed transition-colors"
                >
                  {loading ? 'Updating Video...' : 'Update Video'}
                </button>
                <button
                  type="button"
                  onClick={() => router.push('/admin')}
                  className="px-6 py-2 rounded-md shadow-sm text-sm font-medium text-gray-400 bg-transparent border border-gray-600 hover:bg-gray-800 hover:text-gray-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-700 transition-colors"
                >
                  Cancel
                </button>
              </div>
